...
class PulsarContinuousReader(
  ...
  override def needsReconfiguration(): Boolean = {
    knownTopics != null && pulsarHelper.fetchLatestOffsets().topicOffsets.keySet != knownTopics
  }
  ...
  override def planInputPartitions(start: Offset): Array[InputPartition] = {
    ...
    val currentPartitionSet = pulsarHelper.fetchLatestOffsets().topicOffsets.keySet
    val newPartitions = currentPartitionSet.diff(oldStartPartitionOffsets.keySet)
    val newPartitionOffsets = pulsarHelper.fetchEarliestOffsets(newPartitions.toSeq)
